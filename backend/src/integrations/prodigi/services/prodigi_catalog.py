...
class ProdigiCatalogService:
    ...
    async def refresh_family_cache(self, country: str, prefix: str) -> list[CachedProduct]:
        ...
        async with ProdigiClient() as client:
            ...
            details_list = await client.discover_paper_prints(
                country_code=country, sku_prefixes=[prefix], max_concurrent=15
            )
            ...
            for d in details_list:
                ...
                if " cm" in d.description:
                    try:
                        ...
                        width_cm, height_cm = (
                            round(d.width_in * 2.54, 1),
                            round(d.height_in * 2.54, 1),
                        )
                else:
            ...
            await self.redis.set(
                key, json.dumps([asdict(p) for p in cached_products]), expire=86400
            )
    ...
    def _parse_quote_for_std(self, quote: dict) -> dict[str, float | None]:
        ...
        std_quote = next(
            (q for q in quote["quotes"] if q.get("shipmentMethod") == "Standard"), None
        )
        ...
        if std_quote:
            ...
            prod_cost = sum(
                float(i.get("unitCost", {}).get("amount", 0)) for i in std_quote.get("items", [])
            )
    ...
    async def get_quote_cached(
        self, sku: str, country: str, currency: str = "EUR", attributes: dict | None = None
    ) -> dict | None:
    ...
    async def get_detailed_options(self, country: str, aspect_ratio: str, family_input: str):
        ...
        """
        log.info(
            f"Diagnostic probe: country={country}, ratio={aspect_ratio}, family={family_input}"
        )
        ...
        async with ProdigiClient() as client:
            discovery = await client.discover_paper_prints(
                country_code=country, sku_prefixes=prefixes, max_concurrent=15
            )
            ...
            matched = [
                d for d in matched if any(d.sku.upper().startswith(p.upper()) for p in prefixes)
            ]
                ...
                async with quote_semaphore:
                    try:
                        quote = await client.get_quote(
                            sku_details.sku, country, "EUR", attributes=variant.attributes
                        )
                    except Exception as e:
                        return {
                            "sku": sku_details.sku,
                    ...
                    for q in quote.get("quotes", []):
                        prod_cost = sum(
                            float(i.get("unitCost", {}).get("amount", 0))
                    ...
                    return {
                        ...
                        "is_ideal_match": (
                            variant.attributes.get("paperType") == "HPR"
                            and (
                                # Can be a Box frame or a Premium Framed Print variant
                                variant.attributes.get("frame") == "Box"
                                or "1.4mm" in str(variant.attributes.get("mount", ""))
                            )
                            and (
                                # Support multiple glaze naming conventions
                                "Acrylic" in str(variant.attributes.get("glaze", ""))
                                or "Perspex" in str(variant.attributes.get("glaze", ""))
                            )
                        ),
                        "raw_details": asdict(sku_details),
                        "raw_quote": quote,
            ...
            sorted_results = sorted(
                list(results), key=lambda x: (x["sku"], str(x["applied_attributes"]))
            )
