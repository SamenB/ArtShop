...
function buildShippingSummary(size: StorefrontSizeOption | null): string {
    ...
    if (mode === "pass_through" && customerShipping !== null && customerShipping !== undefined) {
...
export default function PrintConfigurator({
    ...
    const selectedAttributes = useMemo(() => {
        ...
        return (
            attributeSelections[selectedCardKey] || buildInitialAttributeSelection(selectedCard)
        );
    ...
    const preflight = buildPreflightMetrics(storefront, selectedCard, selectedSize);
    ...
    const preflightTone =
        preflight?.status === "ready"
            ...
            : preflight?.status === "caution"
              ? { accent: "#9A6700", bg: "#FFF7E6", border: "rgba(154, 103, 0, 0.15)" }
              : preflight?.status === "insufficient"
                ? { accent: "#B42318", bg: "#FEF3F2", border: "rgba(180, 35, 24, 0.14)" }
                : { accent: "#475467", bg: "#F8FAFC", border: "rgba(71, 84, 103, 0.12)" };
        ...
        <div className="print-configurator-inner">
                            ...
                            {cards.map((card) => (
                                <button
                                    ...
                                    className={`step-option ${
                                        selectedCard?.category_id === card.category_id ? "active" : ""
                                    }`}
                                    onClick={() => {
                    ...
                    <div className="step-row step-reveal" key={key}>
                        ...
                        <div className="step-select-wrap">
                            <button
                                ...
                                type="button"
                            >
                                <span>{formatAttributeValue(selectedAttributes[key] || options[0])}</span>
                                <span className="step-chevron" />
                            </button>
                            <div className={`step-options ${openDropdown === key ? "open" : ""}`}>
                                {options.map((value) => (
                                    <button
                                        key={value}
                                        type="button"
                                        className={`step-option ${
                                            (selectedAttributes[key] || options[0]) === value ? "active" : ""
                                        }`}
                                        onClick={() => {
                ...
                <div className="step-select-wrap">
                    ...
                    >
                        <span>
                            ...
                            {finalPrice !== null ? (
                                <>
                                    {" "}— <span className="font-price font-medium">{convertPrice(finalPrice)}</span>
                                </>
                            ) : null}
                        </span>
                        <span className="step-chevron" />
                        ...
                        {selectedCard?.size_options.map((size) => {
                            ...
                            return (
                                <button
                                    ...
                                    type="button"
                                    className={`step-option ${
                                        selectedSize && getSizeKey(selectedSize) === getSizeKey(size)
                                            ? "active"
                                            : ""
                                    }`}
                                    onClick={() => {
            ...
            <div className="info-badge" style={{ marginTop: "1rem" }}>
                <div className="info-badge-content">
                    ...
                    <p className="info-badge-desc">
                        {routeSummary || "Active baked storefront route is ready for this country."}
                    </p>
                </div>
            </div>
            ...
            <div
                style={{
                    ...
                    display: "grid",
                    gridTemplateColumns: isSmall ? "1fr" : "minmax(0, 1.05fr) minmax(0, 1fr)",
                    gap: "1rem",
                    alignItems: "center",
                }}
            >
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: "0.8rem" }}>
                    ...
                    >
                        <div>
                            ...
                            <p className="info-badge-desc" style={{ color: preflightTone.accent }}>
                        ...
                        <div>
                            ...
                            <p className="info-badge-desc">
                                {preflight ? `${preflight.targetDpi} / ${preflight.minimumDpi} DPI` : "300 / 150 DPI"}
                            </p>
                        </div>
                        <div>
                            <p className="info-badge-title" style={{ marginBottom: "0.15rem" }}>
                            ...
                            <p className="info-badge-desc">{preflight?.frontSizeLabel || "N/A"}</p>
                        </div>
                        <div>
                            <p className="info-badge-title" style={{ marginBottom: "0.15rem" }}>
                                Total Print Area
                    ...
                    <div className="info-badge" style={{ marginTop: 0, background: "rgba(255,255,255,0.62)" }}>
                        <div className="info-badge-content">
